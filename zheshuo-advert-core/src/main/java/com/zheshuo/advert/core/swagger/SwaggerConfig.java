package com.zheshuo.advert.core.swagger;
...
public class SwaggerConfig {
    ...
    @Value("${advert.swagger.title}")
    private String title;
    @Value("${advert.swagger.description}")
    private String description;
    @Value("${advert.swagger.version}")
    private String version;
    @Value("${advert.swagger.enable}")
...
//                .globalOperationParameters(globalParam())
    ...
    private List<SecurityContext> securityContexts() {
        return Collections.singletonList(SecurityContext.builder()
            .securityReferences(Collections.singletonList(new SecurityReference("TokenAuth",
    ...
    public static class ApiDocBuilder implements ApiListingBuilderPlugin, OperationBuilderPlugin {
        ...
        public void apply(OperationContext context) {
            ...
            if (api.value().length() == 0 || api.tags().length == 0 && api.tags()[0].length() == 0) {
                return;
            }
